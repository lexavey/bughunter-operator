...
fast_search(){
    ...
    code=$(curl -o /dev/null -s $url --insecure -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.36 OPR/90.0.4480.100' --connect-timeout 5 --max-time 5 -x https://$host:443 --write-out '%{http_code}')

    echo -ne "$code|$url|$result\r"
    if [[ $code != "000" ]]; then
        echo -ne "$url $code 				OK\n"
        echo "$code|$url">>proxys_$code.txt
    fi
}
