...
fast_search(){
    ...
    NC='\033[0m' # No Color
    host=$1
    code=$(curl -o /dev/null -s "https://$host" -k -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.36 OPR/90.0.4480.100' --connect-timeout 5 --max-time 5 --write-out '%{http_code}')
    if [[ $code == "200" ]]; then
    ...
    echo "$host">>"result/direct_https_status_$code.txt"
}
