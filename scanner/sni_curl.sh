...
fast_search(){
    ...
    NC='\033[0m' # No Color
    host=$1
    code=$(curl --insecure -vvI --connect-to $host:443:sanggoro5.nextvpn.cc:443 --connect-timeout 5 --max-time 5 "https://$host"  2>&1 | awk 'BEGIN { cert=0 } /^\* SSL connection/ { cert=1 } /^\*/ { if (cert) print } '|grep subject
)
    if [ ! -z "${code}" ]; then
